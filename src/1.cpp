...
{
	if((number < length) and dinmass[number]) return dinmass[number];
}

void DinMass::DinMass_Setter(int number, int value)
{
	if((number < length) and (value >= -100) and (value <= 100)) dinmass[number] = value;
}
...
int main(){
	...
	while(count < N)
	{
		...
		if(num_com == 1)
		{
			...
			else cout << dinmass2.DinMass_Getter(num_elem) << "\n";
		}
		if(num_com == 2)
		{
